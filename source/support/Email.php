...
class Email
{
    ...
    {
        try {
            $statement = Connect::getInstance()->prepare("INSERT INTO mail_queue (subject, body, from_email, from_name, recipient_email, recipient_name) VALUES (:subject, :body, :from_email, :from_name, :recipient_email, :recipient_name)");
            $statement->bindValue(":subject", $this->data->subject, \PDO::PARAM_STR);
            $statement->bindValue(":body", $this->data->body, \PDO::PARAM_STR);
            $statement->bindValue(":from_email", $from, \PDO::PARAM_STR);
            $statement->bindValue(":from_name", $fromName, \PDO::PARAM_STR);
            $statement->bindValue(":recipient_email", $this->data->recipient_email, \PDO::PARAM_STR);
            $statement->bindValue(":recipient_name", $this->data->recipient_name, \PDO::PARAM_STR);
    ...
    {
        $statement = Connect::getInstance()->query("SELECT * FROM mail_queue WHERE sent_at IS NULL");
        if ($statement->rowCount()){
            foreach ($statement->fetchAll() as $send){
